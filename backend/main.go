...
func fetchAndStore(symbol string) error {
	...
	stockID, err := upsertStock(symbol)
	if err != nil {
...
func upsertStock(symbol string) (int, error) {
	// Insert the stock if it doesn't exist, return its ID either way
	var id int
	err := db.QueryRow(`
        ...
        RETURNING id
    `, symbol, symbol, "Technology").Scan(&id)
